...
yields <- read_csv("../../data/boc_yield_curves.csv")
yields[, -1] <- 100 * yields[, -1]
...
ggsave(filename = "../../figures/yield_responses_CAN.png", plot = p.all)

p1
ggsave(filename = "../../figures/yield_responses_CAN1.png", plot = p1)

# response to slope factor
responses_slope <- get_term_premium_regression(x = mpu_merged$`2.5` - mpu_merged$`1`, 
                                             y = yields_merged[, -1], 
                                             control = ed_merged$`ED4 Comdty`, 
                                             include.control = T)

p.slope <- ggplot(responses_slope, aes(x = Horizon, y = Beta)) +
  geom_line(aes(y = CILB)) + 
  geom_line(aes(y = CIUB)) + 
  geom_line(size = 1.25)  + 
  geom_ribbon(data=responses_slope, aes(ymin=CILB, ymax=CIUB), fill="grey", alpha="0.25") + 
...
p.all <- gridExtra::grid.arrange(p1, p2, p3, p4)
ggsave(filename = "../../figures/yield_responses_CAN_ed_shock.png", plot = p.all)

p1
ggsave(filename = "../../figures/yield_responses_CAN1_ed_shock.png", plot = p1)
